...
export function IssueDiscussion({ issueId }) {
  ...
  async function handleAddComment() {
    ...
    //   const sentiment = await analyzeSentiment(newComment);
      ...
      if (user && !isAnonymous) {
        await awardPoints(user.id, 'comment_added', ACTIVITY_POINTS.COMMENT_ADDED, issueId);
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h3 className="text-lg font-bold text-gray-800 mb-4">Discussion ({comments.length})</h3>
          ...
          {comments.map((comment) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center space-x-2">
                  <div className="bg-gradient-to-br from-blue-600 to-blue-700 w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm">
                    {comment.is_anonymous
                      ? '?'
                      : comment.commenter?.full_name?.charAt(0).toUpperCase() || 'U'}
                  </div>
                  <div>
                    <p className="font-semibold text-gray-800">
                      {comment.is_anonymous ? 'Anonymous Citizen' : comment.commenter?.full_name || 'User'}
                    </p>
                ...
                {comment.sentiment_score !== null && comment.sentiment_score !== undefined && (
                  <div
                  ...
                  >
                    {comment.sentiment_score > 0.2 ? '😊' : comment.sentiment_score < -0.2 ? '😟' : '😐'}
                  </div>
                )}
              </div>
